...
export const ExportService = {
  async exportToCSV(): Promise<string> {
    ...
    const carbs = StorageService.getCarbEntries();
    ...
    readings.forEach((reading) => {
      ...
      csvContent += `Glucose,${date},${time},${reading.value},${
        reading.unit
      },"${reading.notes || ""}"\n`;
    });
    ...
    shots.forEach((shot) => {
      ...
      csvContent += `Insulin,${date},${time},${shot.units},units,"${
        shot.type
      } - ${shot.notes || ""}"\n`;
    });
    ...
    carbs.forEach((carb) => {
      ...
      csvContent += `Carbs,${date},${time},${carb.carbs},grams,"${
        carb.food
      } - ${carb.notes || ""}"\n`;
    });

    const fileName = `glyco_export_${
